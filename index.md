...
<FeatureCard icon="🧪" title="First-class Testing" details="KafkaBus::fake() intercepts all bus calls — no real broker needed. Assert which messages were published, how many times, and with what payload. Feed messages into the fake consumer and assert commits. Available in kafka-bus/laravel-bridge only." link="/docs/laravel/testing">
...
<FeatureCard icon="🟥" title="Laravel Integration" details="Install kafka-bus/laravel-bridge for auto-discovery, a publishable config file, and Artisan commands. The KafkaBus facade mirrors the core Bus API and adds a fake() method that works exactly like Event::fake() in your feature tests." link="/docs/laravel/installation">
